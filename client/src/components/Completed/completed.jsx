...
const Completed = () => {
    ...
    useEffect(() => {
        ListTaskByStatus("completed");
    },[])
    const CompletedList = useSelector((state) => state.task.Completed)
    const DeleteItem = (id) => {
        DeleteTodo(id).then((result) => {
            if (result===true) {
                // dispatch(setCompleted(CompletedList.filter(task => task._id !== id)));
                ListTaskByStatus("completed");
            }
        })
    }
                                ...
                                <div className="card h-100 border-0 shadow-sm">
                                    <div className="card-body">
                                        ...
                                        <p className="m-0 p-0 animate__animated animate__fadeInUp">
                                            ...
                                            <a className="badge float-end bg-success text-decoration-none text-white">{item.status}</a>
                                        </p>
                                    </div>
                                </div>
                            </div>
